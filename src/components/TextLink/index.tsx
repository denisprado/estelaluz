...
export default function TextLink({ url, text, className }: { url: string | Href, text: string, className?: string }) {
	return (
		<Link href={url} className={classNames("font-medium transform transition duration-500  opacity-100 hover:opacity-80 block py-2 px-3 text-gray-900 rounded  md:hover:bg-transparent md:hover:text-gray-700 md:p-0 md:dark:hover:text-gray-500 dark:text-white dark:hover:text-white md:dark:hover:bg-transparent", className)}>{text}</Link>
	)
}