...
const ProductCategories = (props: { productCategories: CategoryProduct[]; }) => {
	...
	return (
		<div className="flex flex-col sm:flex-row gap-4">
			<Link href={'/adquira/todos'} className={classNames("uppercase font-bold hover:underline-offset-2 hover:underline",pathName==='/adquira/todos'&& 'underline-offset-2 underline')}>todos</Link>
			{props.productCategories.map(cat => {
				const isActive = pathName === "/adquira/" + cat.slug
				console.log(isActive, pathName, "/adquira/" + cat.slug)
